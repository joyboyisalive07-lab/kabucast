...
**На телефоне.** Откройте <https://joyboyisalive07-lab.github.io/kabucast/> и
добавьте на домашний экран. Дальше открывается как приложение и работает без
сети. Так и стоит пользоваться большинству: ставить нечего и доверять нечему.

**На компьютере.** Тот же адрес. Либо скачайте `kabucast-offline.html` из
[последнего релиза](https://github.com/joyboyisalive07-lab/kabucast/releases/latest):
один файл, около 83 КБ, всё внутри, открывается прямо с диска.

**Программой под Windows.** `kabucast.exe` из того же релиза несёт эту страницу
и открывает её в браузере по умолчанию. Про предупреждение Windows — ниже.
...
так что кнопка копирования даёт то, что можно кому-нибудь отправить, а
присланная вам ссылка открывается сразу на прогнозе, а не на пустых полях.

График доступен без мыши: дайте ему фокус, и стрелки проходят его по полудням,
зачитывая точные числа. У него есть текстовое описание недели для тех, кто его
не видит.

Семь языков: английский, русский, немецкий, испанский, французский, японский,
китайский упрощённый.

## Почему Windows ругается на исполняемый файл

Потому что он не подписан, и об этом честнее сказать прямо, чем обходить.

Чтобы предупреждение исчезло, нужен сертификат подписи кода, выданный на
проверенное юридическое лицо. Самоподписанный сертификат не даёт ничего:
предупреждение существует именно потому, что подписи, ручающейся за саму себя,
никто не доверяет. Упаковать программу в zip значило бы спрятать предупреждение
браузера, не изменив того, что вы запускаете, — это было бы хуже самого
предупреждения.

Взамен проект даёт проверяемое происхождение. Каждый релиз собирается публично
рабочим процессом из этого репозитория, и к каждому артефакту прикладывается
аттестация сборки:
...
Это сверяет файл с тем прогоном, который его произвёл. `SHA256SUMS.txt` в релизе
позволяет убедиться, что скачалось без искажений.

Если и этого мало — не запускайте. `kabucast-offline.html` — тот же инструмент,
не вызывает предупреждений нигде и работает на любой платформе, включая телефон,
на котором exe не запустится никогда.
...
MIT, правообладатель joy boy. См. [LICENSE](LICENSE).
